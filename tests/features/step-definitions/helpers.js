...
export async function getWhereIAm(world) {
  ...
  //a shorted checking image source flow:
  let imageSource;
  while (true) {
    imageSource = await world.get('main .big-image')
      .getAttribute('src');
    if (imageSource) { break; }
    await world.sleep(20);
  } 
  let imageName = imageSource.split('/')[1].split('.')[0];
...
export async function navigateTo(world, to) {
  ...
  /* we navigate to outside the cafe first and then continue to   where we want to be (outside the cafe is like a bus hub)*/
  ...
  let choicesToMake = [
    ...
    // Above will resault simular to: ['from destination', 'to destination']
  ];
